...
sz? size = aiofiles::file_size(String path);
AioFile? afile = aiofiles::open(String path, String mode);
void? afile.close();

sz? n = afile.read_unbuffered(char[] buf); // .seek(), .tell() compatible
sz? n = afile.read(char[] buf); // buffered, not compatible with .seek() and .tell();
sz? n = afile.read_until(char delimiter, char[] buf);

bool b = afile.at_eof();

sz? n = afile.write(char[] buf);
sz? n = afile.writef(String format, ...args);
void afile.flush();
...
sz? os::sendfile(int to_sock, int from_fd, long offset, long count);
...
sz? n = os::readlink(Stri g path, char[] buf);
...
sz? size = path::getsize(String path);
...
* [../../test/aiofiles/aio_sample.c3](../../test/aiofiles/aio_sample.c3)
* [../../test/aiofiles/aio_tempfile.c3](../../test/aiofiles/aio_tempfile.c3)
