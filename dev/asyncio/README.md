...
void asyncio::set_eventloop_config(sz taskq=1024, sz timerq=512, sz soonq=512); // embedded server: 64,16,32, busy server:4096,2048,2048
...
alias CompletedCallback = fn void(Task*, sz);

void? asyncio::as_completed(Task** tasks, sz count, CompletedCallback cb) @maydiscard; // Invoke cb in completion order. Returns CANCELLED_ERR if the calling task is cancelled while waiting.
...
sz num_removed = future.remove_done_callback(Callback cb);
...
sz? asyncio::wait_any(Future** futs, sz count) @maydiscard; // Return the index of the first Future to complete. Returns CANCELLED_ERR if the calling task is cancelled while waiting.
```
...
Task* task = asyncio::EventLoop.create_task(&self, Coroutine coro, String name = "", sz stack_size = 128_000, bool autofree = false, void* arg = null);
...
Queue* queue = asyncio::queue_new(sz capacity = QUEUE_CAP_DEFAULT);
void queue.free(); // prevent leak 
sz n = queue.size();
bool b = queue.empty();
bool b = queue.full();
sz n = queue.maxsize(); // capacity
...
sz? n = asyncio::async_read(int fd, void* buf, sz len);

sz? n = asyncio::async_write(int fd, void* buf, sz len);

sz? n = asyncio::async_recvfrom(int fd, void* buf, sz len, char* ip_str, sz* ip_len, ushort* port); // UDP

sz? n = asyncio::async_sendto(int fd, void* buf, sz len, char* ip_str, ushort port); // UDP
...
sz? n = stream.read(char[] buf); 

sz? n = stream.read_any(char[] buf);

sz? n = stream.read_until(char delim, char[] buf);

sz? n = stream.readline(char[] buf);

sz? n = stream.write(char[] buf);

sz? n = stream.writef(String fmt, args...); 
...
sz n = dgram.sendto(char[] data, char[] ip, short port);

sz n = dgram.recvfrom(char[] data, char[] ip, sz* ip_len, ushort* port);

sz n = loop.sock_sendto(&self, int sock_fd, char[] data, char[] ip, short port);

sz n = loop.sock_recvfrom(&self, int sock_fd, char[] data, char[] ip, sz* ip_len, ushort* port);
...
sz? n = trans.write(char[] data); // tcp
